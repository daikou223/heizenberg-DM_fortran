program baseEnegy
    ...
    integer::mask,key,upperUseKey,oneNum
        ...
        do UP_NUM = N,1,-1
            ...
            if(UP_NUM <= DOWN_NUM) then
                ...
                do l1 = 1,ALL_STATE_NUM
                    ...
                    key = ishft(states(l1),-1)
                    mask = ishft(1,N-1)-1
                    do L2 = 1,size(collapseList)
                        if(idToInd(key) > 0) then
                            key = ieor(ishft(states(l1),-1),iand(mask,collapseList(L2)))
                        ...
                        do restStatesInd = 1,restStatesNum
                            ...
                            do l1 = 1,size(collapseList)
                                if(States(idToInd(key)) == state) then
                                    ...
                                    key = ieor(ishft(state,-1),iand(mask,collapseList(L1)))
                                end if
                            end do 
                            ...
                            do l1 = 1,size(collapseList)
                                if(States(idToInd(key)) == fripState) then
                                    ...
                                    key = ieor(ishft(fripState,-1),iand(mask,collapseList(L1)))
                                end if
                            end do 
