...
contains
    ...
    function getbit(num,rank) result(bit)
        integer,intent(in)::num,rank
        integer::bit
        bit = merge(1, 0, btest(num, rank))
    end function getbit

    !ある値のrank番目にinsertBitを挿入した値を返却
    function setbit(num,rank,insertBit) result(newnum)
        integer, intent(in) :: num,rank,insertBit
        integer::newnum,mask,maskedNum
        mask = 1
        mask = ishft(mask,rank)
        mask = mask -1
        maskedNum = iand(num,mask)
        if(insertBit == 1)then
            newnum = ior(ishft(ibset(ishft(num,-rank+1),0),rank),maskedNum)
        else
            newnum = ior(ishft(ibclr(ishft(num,-rank+1),0),rank),maskedNum)
        end if
