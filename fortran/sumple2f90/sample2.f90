...
!**********
 module com
!**********
 integer,parameter::NSITE=8,NPAIR=8,SZTOT=0
...
!*************
 ...
 implicit none
!*************
 external set_pair,diag
 external set_stno,set_element_sz
 integer,external::nCr
 integer::dim,ne,nvec,ns,np,i,pair(0:NPAIR-1,2),r
 real*8::dummy(1),eps
 real*8,dimension(:,:),allocatable::element,wk
 real*8,dimension(:),allocatable::ev
 integer,dimension(:),allocatable::iwk,stno,cfg

   ns=NSITE
   np=NPAIR
   call set_pair(ns,np,pair)

   r=ns/2+SZTOT !アップスピンの総数
   dim=nCr(ns,r)
   allocate(stno(0:2**ns-1),cfg(0:dim-1))
   call set_stno(ns,dim,r,stno,cfg)

   allocate(element(dim,dim))
   call set_element_sz(ns,np,dim,pair,stno,cfg,element)

   allocate(wk(dim,8),ev(dim),iwk(dim))
   ne=dim
   nvec=0
   eps=CEPS
   ...
   open(1,FILE="sample2.dat")
   write(1,'("ns=",I3," sztot=",I3," dim=",I8)') ns,SZTOT,dim
   do i=1,dim
      write(1,'(E23.15)') ev(i)
   enddo
   close(1)
   ...
   deallocate(stno,cfg,element,wk,ev,iwk)
   contains
 end program main
...
!*************************************************
 subroutine set_stno(ns,dim,r,stno,cfg) !24.3.22
 implicit none
 integer,intent(in)::ns,dim,r
 integer,intent(out)::stno(0:2**ns-1),cfg(0:dim-1)
 integer disp
!*************************************************
 integer::i,pt,cnt,r0
  ...
  open(2,file = "spin4.txt")
   write(2,'(I10)') dim
   cnt=0
   do i=0,2**ns-1
      r0=0
      do pt=0,ns-1
         if(ibits(i,pt,1)==1) r0=r0+1
      enddo
      if(r0==r) then
        do disp = ns-1,0,-1
            write(2,'(I1)',advance="no") ibits(i,disp,1)
          end do
          write(2,"(A)")" "
      if(cnt>dim-1) then
            write(*,*) "err in set_stno"
      stop
      endif
         cfg(cnt)=i
         stno(i)=cnt
 ...
 end subroutine set_stno

!*******************************************************************
 subroutine set_element_sz(ns,np,dim,pair,stno,cfg,element) !24.3.22
 implicit none
 integer,intent(in)::ns,np,dim
 integer,intent(in)::pair(0:np-1,2),stno(0:2**ns-1),cfg(0:dim-1)
